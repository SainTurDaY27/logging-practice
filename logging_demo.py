...
def logging_test(logger):
    ...
    # warning
    logger.warning("Please pay attention")
...
def my_config():
    ...
    FORMAT = '%(message)s [%(name)s / %(levelname)s] at %(asctime)s  '
    logging.basicConfig(format=FORMAT, level=logging.NOTSET, filemode='w', filename='data_logging')
